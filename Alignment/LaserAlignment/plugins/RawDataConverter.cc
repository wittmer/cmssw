...
RawDataConverter::RawDataConverter( const edm::ParameterSet& iConfig ) :
  ...
  orbitNumber(-1)  
{
  sort_output = iConfig.getParameter<bool>( "SortOutput" );
  output_filename = iConfig.getUntrackedParameter<std::string>( "OutputFileName" );
  theOutputFile = new TFile( output_filename.c_str() , "RECREATE" );
  theOutputFile->cd();
  ...
  theOutputTree->Branch( "orbitNumber", &orbitNumber, "orbitNumber/I" );
  ...
  theOutputTree->Branch( "microsecondOffset", &microsecondOffset, "microsecondOffset/i" );
...
void RawDataConverter::analyze( const edm::Event& iEvent, const edm::EventSetup& iSetup )
{
  ...
  microsecondOffset = timestamp.microsecondOffset();
  ...
  theOutputTree->Fill();
  if(sort_output)sort_index.push_back( std::pair<int,int>(eventnumber, entry++));
...
///
/// If requested, sort the events by ascending eventnumber  and write them to another output file
///
void RawDataConverter::endJob()
{
  if(sort_output){

    edm::LogInfo("RawDataConverter") << "Create the Index";
    // Sort the index to get the events in ascending eventnumber
    struct sort_pred {
    ...
    std::sort(sort_index.begin(), sort_index.end(), sort_pred());
    
    edm::LogInfo("RawDataConverter") << "Index Created, reshuffle the tree";
    // Write the events in sorted order to the final file
    TFile SortedFile( ( output_filename + ".sorted" ).c_str() , "RECREATE" );
    SortedFile.cd();

    TTree *tsorted = (TTree*)theOutputTree->CloneTree(0);

    for (std::vector< std::pair<int,int> >::iterator it = sort_index.begin(); it != sort_index.end(); it++) {
      theOutputTree->GetEntry(it->second);
      tsorted->Fill();
    }
  ...
  theOutputFile->Write();
  theOutputFile->Close();
  edm::LogInfo("RawDataConverter") << "RawDataConverter has finished";
}
